...
%Gerar a palavra de 16 bits para ser codificada
    ...
    for i= 1:num_testes
        ...
        pcode=zeros(4,8);
        perros=zeros(1,32);
        pcode=encoder32(entrada)

        pcode=reshape(pcode',1,32);%%converte a matriz
        %para vetor, assim fica mais fácil inserir os erros 
        perros=pcode;
        mperros=zeros(4,8);%matriz de erros

        %%inserir tipos de falhas dependendo do número inserido
        switch num_falhas
            case 1
                flag='Erro em um bit'
                poserro=randi(32,1,1)%%posição do erro randomicamente escolhida
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%tabela1%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                vp=[-9 -8 -7 -1 1 7 8 9]; %%vp(round((length(vp)-1)*rand(1,1))+1) me dará um elemento aleatório que representaria a posição onde a matriz teria o segundo erro
            ...
            case 7
                ...
                end                %%perros(poserro)=~perros(poserro);
                num=vp(round((length(vp)-1)*rand(1,1))+1)
                num2=vp(round((length(vp)-1)*rand(1,1))+1)
                num3=vp(round((length(vp)-1)*rand(1,1))+1)
                num4=vp(round((length(vp)-1)*rand(1,1))+1)
...
%         [saidacode,numdetec]=DecodificadorCLC(perros)
        [saidacode,numdetec]=decoder32(perros)
        if(length(vet)==length(vetunique))
